...
if ! ./cross setup >>"${RESULT_DIR}/crossfile-003-bash.log" 2>&1; then
    rc=$?
    if [[ ${rc} -ne 1 ]]; then
        cat "${RESULT_DIR}/crossfile-003-bash.log" >&2
        exit ${rc}
    fi
fi
FETCHED=('')
...
for spec in "khue:metal deploy/metal" "bill:setup/flux deploy/flux" "core:asciinema asciinema"; do
    if ! ./cross patch ${spec} >>"${RESULT_DIR}/crossfile-003-bash.log" 2>&1; then
        rc=$?
        if [[ ${rc} -ne 1 ]]; then
            cat "${RESULT_DIR}/crossfile-003-bash.log" >&2
            exit ${rc}
